...
async def dato(event):
    ...
    e69rr = await event.get_reply_message()
    pic = await e69rr.download_media()
    await bot.send_file(
        "me",
        pic,
        caption="""
...
async def krar(event, caption):
    media = await event.download_media()
    sender = await event.get_sender()
    sender_id = event.sender_id
    e69rr_date = event.date.strftime("%Y-%m-%d")
    e69rr_day = Aljoker_Asbo3[event.date.strftime("%A")]
    await bot.send_file(
        "me",
        media,
        caption=caption.format(sender.first_name, sender_id, e69rr_date, e69rr_day),
        parse_mode="markdown"
    )
    os.remove(media)
